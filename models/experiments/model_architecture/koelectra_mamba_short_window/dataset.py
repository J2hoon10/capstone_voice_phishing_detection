"""
koelectra_mamba_short_window / dataset.py

hce_ordinal 대비 핵심 변경:
  - WINDOW_SIZE=64, STRIDE=32 슬라이딩 윈도우 (세그먼트 수 ~3배 증가)
  - segment_risks: 기존 CSV 컬럼(WINDOW=128 기준) 대신
    segment_risks_w64_s32.csv (WINDOW=64, STRIDE=32 기준)를 id로 lookup
    → 실제 윈도우 크기에 맞는 위험도 레이블 사용

사전 준비 (Linux capstone 환경):
  cd models/classifier/preprocessing/stn_labeling
  python map_segments.py --window-size 64 --stride 32 \
      --output-csv output/segment_risks_w64_s32.csv
"""
...
def load_segment_risks_lookup(csv_path: Path) -> dict[str, list[int]]:
    """
    segment_risks_w64_s32.csv → {conv_id: [risk0, risk1, ...]} (0-indexed)
    파일이 없으면 빈 dict 반환 (train 전에 map_segments.py를 먼저 실행해야 함).
    """
...
def build_segments(tokenizer, text: str) -> list[dict]:
    # WINDOW_SIZE=64, STRIDE=32 슬라이딩 윈도우
    token_ids = tokenizer(
        text,
        ...
        with self.csv_path.open("r", encoding="utf-8-sig") as f:
            for row in csv.DictReader(f):
                ...
                if conv_id in risks_lookup:
                    ...
                    # 세그먼트 수가 다를 경우 truncate/pad (토크나이저 동일하므로 대부분 일치)
                    if len(raw) >= len(segs):
