"""
koelectra_mamba_diff_lr / train.py

koelectra_mamba_hce_ordinal 대비 주요 변경 (학습 전략 최적화 실험 B):
  - 차등 학습률(Differential Learning Rate) 전략 적용
  - KoELECTRA: ENCODER_LR=1e-5 (사전학습 지식 보호, Catastrophic Forgetting 방지)
  - Mamba + Head: UPPER_LR=1e-3 (무작위 초기화 파라미터, 빠른 수렴 유도)
  - 기존 koelectra_mamba_hce_ordinal 대비 UPPER_LR을 크게 높여
    두 파트의 학습 밸런스를 맞추는 것이 핵심 차이점
  - 동결 단계 없이 처음부터 두 파트를 동시에 학습하되 LR만 차등 적용
"""
...
EXP_NAME = "koelectra_mamba_diff_lr"
...
def build_optimizer(model: nn.Module) -> AdamW:
    """
    차등 학습률 옵티마이저.
    - encoder.*: ENCODER_LR (1e-5) — 사전학습 지식 보존
    - 나머지(Mamba, Head): UPPER_LR (1e-3) — 빠른 수렴
    """
    enc, upper = [], []
        ...
        with self.text_path.open("a", encoding="utf-8") as f:
            ...
            if t == "step":
                f.write(
                    ...
                    f"enc_lr={record['enc_lr']:.2e} "
                    f"upper_lr={record['upper_lr']:.2e} "
...
def run_epoch(
    ...
    for batch in loader:
        ...
        if global_step % LOG_STEP_INTERVAL == 0:
            ...
            step_loss_buf.clear()
            # 두 파라미터 그룹의 LR을 각각 기록
            enc_lr = optimizer.param_groups[0]["lr"] if len(optimizer.param_groups) > 0 else 0.0
            ...
            print(
                ...
                f"loss={avg_loss:.4f} enc_lr={enc_lr:.2e} "
                f"upper_lr={upper_lr:.2e} beta={beta:.3f} elapsed={elapsed:.0f}s"
            )
...
def train(resume: bool = False) -> None:
    ...
    print(
        f"[train] 차등 학습률 — "
        f"ENCODER_LR={TRAIN_CONFIG['ENCODER_LR']:.2e} (KoELECTRA), "
        f"UPPER_LR={TRAIN_CONFIG['UPPER_LR']:.2e} (Mamba+Head)"
    )
    print(
