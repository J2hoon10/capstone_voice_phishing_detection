"""
SNS dialogue preprocessing for Compressive Memory KR experiments.

Pipeline:
1. Recursively load normal_data_reference/Training/**/*.json
2. Normalize subject labels and tokenize all turns
3. Stratified split Training -> train/val (90/10)
4. Recursively load normal_data_reference/Validation/**/*.json -> test
5. Save train/val/test cache and label/class-weight metadata
"""
...
def preprocess_json_file(file_path: Path, source_root: Path, source_split: str, tokenizer) -> tuple[dict | None, str | None]:
    try:
...
def load_split_dialogues(source_root: Path, source_split: str, tokenizer) -> tuple[list[dict], dict]:
    json_files = sorted(source_root.rglob("*.json"))
    processed: list[dict] = []
    skipped = Counter()

    for fp in json_files:
        item, err = preprocess_json_file(fp, source_root=source_root, source_split=source_split, tokenizer=tokenizer)
        if err is not None:
            skipped[err] += 1
            continue
...
def build_stratified_train_val(training_dialogues: list[dict]) -> tuple[list[dict], list[dict]]:
...
def compute_class_weights(train_dialogues: list[dict]) -> dict:
    ...
    if y.size == 0:
        ...
        class_weight = sk_compute_class_weight(class_weight="balanced", classes=classes, y=y)
...
def main() -> None:
    ...
    raw_training, train_info = load_split_dialogues(train_root, source_split="training", tokenizer=tokenizer)
    ...
    test_data, test_info = load_split_dialogues(test_root, source_split="validation", tokenizer=tokenizer)
    ...
    save_json(class_weight, DATA_DIR / "class_weight.json", pretty=True)
