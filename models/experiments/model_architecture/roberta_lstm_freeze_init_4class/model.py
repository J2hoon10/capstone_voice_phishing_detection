"""
roberta_lstm_freeze_init_4class / model.py

roberta_mamba_freeze_init_4class 와 동일한 구조에서
Mamba SSM을 2-layer 단방향 LSTM으로 교체.
  ...
  - LSTM(input=768, hidden=768, num_layers=2, dropout=0.1) — 단방향
  - HierarchicalHead: 일반(2)/피싱(2) superclass → 세부 클래스 계층적 분류
  - aux_head: 세그먼트별 이진 위험도 (LSTM 입력 전 raw 벡터에서 예측)
...
class HierarchicalHead(nn.Module):
    """일반(2) / 피싱(2) superclass → subclass 계층적 분류 헤드."""
...
class StreamingBeliefClassifier(nn.Module):
    """RoBERTa + AttentionWeightedPooling + 2-layer LSTM + HierarchicalHead."""
    ...
    def __init__(
        ...
        self.aux_head = nn.Linear(d_model, 2)  # LSTM 이전 raw 세그먼트 벡터에서 예측
    ...
    def forward(
        ...
        **kwargs,
        ...
        aux_logits = self.aux_head(x_stacked)             # (B, T, 2) — LSTM 전 raw 벡터
        ...
        all_logits = [torch.log(logits_4 + 1e-8)] * T

        return {
            "logits": torch.log(logits_4 + 1e-8),
