...
def build_segments(tokenizer, text: str) -> list[dict]:
    # KoBERT(KoBertTokenizer)는 slow tokenizer라 return_offsets_mapping 미지원
    # → 토큰 문자열 기반으로 SentencePiece ▁ prefix를 이용해 단어 경계 판단
    ...
    # SentencePiece: ▁ prefix가 있으면 새 단어 시작
    def _spm_is_word_start(i: int) -> bool:
        t = token_strings[i]
        return i == 0 or t.startswith("▁") or t.startswith("▂")

    # 문장 경계: 마침표/물음표/느낌표/줄바꿈이 포함된 토큰 이후를 경계로 간주
    def _spm_sentence_ranges() -> list[tuple[int, int]]:
        _SENT_END = re.compile(r"[.?!\n]")
        sentences, s_start = [], 0
        for i, t in enumerate(token_strings):
            if _SENT_END.search(t.replace("▁", "").replace("▂", "")):
                sentences.append((s_start, i + 1))
                s_start = i + 1
        if s_start < n:
            sentences.append((s_start, n))
    ...
    sentences = _spm_sentence_ranges()
    token_to_sent = [0] * n
    ...
    for start in starts:
        ...
        # ── 단어 경계 조정 (SentencePiece ▁ prefix 기반) ────────────────
        if end < n and not _spm_is_word_start(end):
            while end < n and not _spm_is_word_start(end):
                end += 1
...
class CsvStreamingDataset(Dataset):
    def __init__(self, csv_path: str | Path):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(ENCODER_CONFIG["MODEL_NAME"], trust_remote_code=True)
        self.rows = []
