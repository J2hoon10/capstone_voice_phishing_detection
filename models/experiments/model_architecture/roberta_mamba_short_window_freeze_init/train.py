"""
roberta_mamba_short_window_freeze_init / train.py

freeze_init 전략:
  epoch 1 ~ FREEZE_INIT_EPOCHS : encoder 전체 freeze → Mamba + head 만 학습
  epoch FREEZE_INIT_EPOCHS+1 ~ : encoder layer[10~11] unfreeze → encoder 상위 레이어 fine-tune
  embeddings + layer[0~9] 는 끝까지 freeze 유지

핵심 구현:
  - build_model(unfreeze_all=True) 로 모든 파라미터 trainable 상태로 생성
  - build_optimizer() 호출 → encoder 파라미터도 optimizer에 포함
  - freeze_encoder_all() 로 encoder requires_grad=False
    (optimizer에는 이미 등록된 상태 → unfreeze 시 add_param_group 불필요)
  - unfreeze 시점에 requires_grad=True 복원 → optimizer가 자동으로 업데이트 재개
"""
...
EXP_NAME = "roberta_mamba_short_window_freeze_init"
...
def train(resume: bool = False) -> None:
    ...
    print(
        f"[train] WINDOW_SIZE={TRAIN_CONFIG.get('WINDOW_SIZE', 64)} STRIDE={TRAIN_CONFIG.get('STRIDE', 32)} "
        f"BATCH_SIZE={TRAIN_CONFIG['BATCH_SIZE']} "
        f"NUM_LAYERS=1 freeze_init_epochs={FREEZE_INIT_EPOCHS}"
    )
    print(
