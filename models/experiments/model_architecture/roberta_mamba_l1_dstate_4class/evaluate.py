"""
roberta_mamba_l1_dstate_4class / evaluate.py

사용 예:
  python evaluate.py --d-state 16
  python evaluate.py --d-state 32 --split val
  python evaluate.py --d-state 64 --run-id 20260517_120000
"""
...
def resolve_checkpoint_path(exp_name: str, run_id: str | None = None) -> tuple[Path, str]:
...
def print_results(results: dict, exp_name: str, split: str, run_id: str, d_state: int) -> None:
    print("\n" + "=" * 60)
    print(f"  {exp_name}  D_STATE={d_state}  split={split}  run={run_id}  ({results['num_samples']} samples)")
    print("=" * 60)
...
def main() -> None:
    parser = argparse.ArgumentParser(description=f"Evaluate {BASE_EXP_NAME}")
    parser.add_argument(
        "--d-state", type=int, default=cfg.MAMBA_CONFIG["D_STATE"],
        help=f"Mamba D_STATE (기본값: {cfg.MAMBA_CONFIG['D_STATE']}). 학습 시 사용한 값과 일치해야 한다.",
    )
    parser.add_argument("--split", default="test", choices=["val", "test", "ood"])
    parser.add_argument("--run-id", default=None, help="특정 run_id 지정 (없으면 latest 사용)")
    parser.add_argument("--ckpt", default=None, help="체크포인트 파일 경로 직접 지정 (다른 실험 결과 평가 시)")
    args = parser.parse_args()

    # ── D_STATE 런타임 오버라이드 ────────────────────────────────────────────
    cfg.MAMBA_CONFIG["D_STATE"] = args.d_state
    exp_name = f"{BASE_EXP_NAME}_ds{args.d_state}"

    if args.ckpt is not None:
        ckpt_path = Path(args.ckpt)
        run_id = ckpt_path.stem
    else:
        ckpt_path, run_id = resolve_checkpoint_path(exp_name, args.run_id)
    if not ckpt_path.exists():
        raise FileNotFoundError(f"checkpoint not found: {ckpt_path}")

    print(f"[eval] checkpoint: {ckpt_path}  (run={run_id}, d_state={args.d_state})")
    ...
    results["d_state"] = args.d_state
    print_results(results, exp_name, args.split, run_id, args.d_state)
