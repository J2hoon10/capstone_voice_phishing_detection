"""
roberta_mamba_l1_dstate_4class / model.py

roberta_mamba_freeze_init_4class 와 동일한 아키텍처.
  - 인코더: KLUE-RoBERTa-base (klue/roberta-base)
  - NUM_LAYERS=1 고정 (freeze_init_4class 의 L2 대비 L1)
  - D_STATE: config.MAMBA_CONFIG["D_STATE"] 를 사용 — train.py/evaluate.py 에서
    --d-state 인수로 런타임 오버라이드 후 build_model() 을 호출할 것
...
def build_model() -> StreamingBeliefClassifier:
    ...
    # encoder freeze/unfreeze 는 train.py 의 freeze_init 전략으로 관리
    # D_STATE: train.py/evaluate.py 에서 --d-state 로 MAMBA_CONFIG["D_STATE"] 를
    #          오버라이드한 뒤 이 함수를 호출해야 한다
