"""
roberta_gru_freeze_init_4class / losses.py

roberta_mamba_freeze_init_4class 와 동일한 손실 함수.
1. OrdinalRegressionLoss  — 세그먼트별 위험도 보조 손실
2. HierarchicalCrossEntropyLoss — superclass + subclass 계층적 분류 손실
"""
...
class HierarchicalCrossEntropyLoss(nn.Module):
    ...
    def forward(
        ...
        if phishing_mask.any():
            ...
            p_targets = targets[phishing_mask] - 2  # label 2,3 → 0,1
            if self.focal_gamma > 0:
