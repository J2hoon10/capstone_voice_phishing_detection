"""
roberta_lstm_short_window_freeze_init / train.py

freeze_init 전략:
  epoch 1 ~ FREEZE_INIT_EPOCHS : encoder 전체 freeze → LSTM + head 만 학습
...
EXP_NAME = "roberta_lstm_short_window_freeze_init"
...
def train(resume: bool = False) -> None:
    ...
    print(
        f"[train] WINDOW_SIZE=64 STRIDE=32 "
        f"BATCH_SIZE={TRAIN_CONFIG['BATCH_SIZE']} "
        f"LSTM_NUM_LAYERS=2 freeze_init_epochs={FREEZE_INIT_EPOCHS}"
    )
    print(
