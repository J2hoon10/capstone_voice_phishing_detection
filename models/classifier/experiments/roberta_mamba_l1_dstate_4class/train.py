"""
roberta_mamba_l1_dstate_4class / train.py

D_STATE ablation 실험용 학습 스크립트.
  --d-state 인수로 Mamba 내부 상태 크기를 런타임에 지정한다.
  지정하지 않으면 config.MAMBA_CONFIG["D_STATE"] 기본값(16)을 사용한다.

사용 예:
  python train.py --d-state 16
  python train.py --d-state 32
  python train.py --d-state 64
...
스케줄러:
  - encoder group : 1e-5 → cosine → 1e-6
  - Mamba+Head group : 5e-4 → cosine → 1e-4
"""
...
BASE_EXP_NAME = "roberta_mamba_l1_dstate_4class"
        ...
        with self.text_path.open("a", encoding="utf-8") as f:
            ...
            if t == "step":
                ...
                f.write(
                    ...
                    f"d_state={record['d_state']} dataset={record['dataset']}\n"
                )
...
def run_epoch(
    ...
    step_aniso_buf: list[float] = []
    all_aniso: list[float] = []
...
def train(d_state: int, resume: bool = False) -> None:
    # ── D_STATE 런타임 오버라이드 ────────────────────────────────────────────
    cfg.MAMBA_CONFIG["D_STATE"] = d_state
    exp_name = f"{BASE_EXP_NAME}_ds{d_state}"
    ...
    if resume and RESUME_CKPT.exists():
        ...
        # resume 시 저장된 d_state 와 현재 인수가 다르면 경고
        saved_ds = ckpt.get("d_state", -1)
        if saved_ds != d_state:
            print(f"[warn] resume checkpoint d_state={saved_ds} != --d-state={d_state}")
        ...
        run_id = ckpt["run_id"]
        print(f"[resume] epoch {start_epoch}부터 재개 (global_step={global_step}, best_f1={best_f1:.4f})")
    elif resume:
    ...
    if start_epoch <= FREEZE_INIT_EPOCHS:
        ...
        print(f"[freeze_init] encoder 완전 freeze (epoch {start_epoch}~{FREEZE_INIT_EPOCHS})")
    else:
    ...
    print(
        f"[train] D_STATE={d_state} NUM_LAYERS=1 "
        f"WINDOW_SIZE={WINDOW_SIZE} STRIDE={STRIDE} "
        f"BATCH_SIZE={TRAIN_CONFIG['BATCH_SIZE']} "
        f"freeze_init_epochs={FREEZE_INIT_EPOCHS}"
    )
    print(
        f"[train] encoder_lr={TRAIN_CONFIG['ENCODER_LR']:.0e}→{TRAIN_CONFIG['ENCODER_MIN_LR']:.0e} "
        f"upper_lr={TRAIN_CONFIG['UPPER_LR']:.0e}→{TRAIN_CONFIG['UPPER_MIN_LR']:.0e} (cosine)"
    )
    print(f"[train] epochs={start_epoch}~{total_epochs}  steps_per_epoch≈{len(train_loader)}  device={DEVICE}")
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        save_resume_ckpt(model, optimizer, scheduler, epoch, global_step, best_f1, best_path, run_id, d_state)

    latest_meta = CHECKPOINT_DIR / f"{exp_name}_latest.json"
    with latest_meta.open("w", encoding="utf-8") as f:
        json.dump(
            {"checkpoint_path": str(best_path), "best_macro_f1": best_f1, "run_id": run_id, "d_state": d_state},
            f, ensure_ascii=False, indent=2,
        )

    print(f"[done] d_state={d_state}  best_f1={best_f1:.4f}  log={logger.text_path}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--d-state", type=int, default=cfg.MAMBA_CONFIG["D_STATE"],
        help=f"Mamba D_STATE (기본값: {cfg.MAMBA_CONFIG['D_STATE']}). 실험 예: 16 / 32 / 64",
    )
    parser.add_argument(
    ...
    train(d_state=args.d_state, resume=args.resume)
