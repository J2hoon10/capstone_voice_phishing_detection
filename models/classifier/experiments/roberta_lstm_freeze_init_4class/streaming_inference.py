"""
streaming_inference.py  (roberta_lstm_freeze_init_4class)
...
EXP_NAME = "roberta_lstm_freeze_init_4class"
LABEL_COLORS = {
    "상담 대화":      "\033[94m",   # 파랑
    "일상 대화":      "\033[92m",   # 초록
    "대출 사기형":    "\033[93m",   # 노랑
...
def predict_from_rnn_out(model, rnn_out_last: torch.Tensor, temp: float = 1.0) -> torch.Tensor:
    """LSTM 마지막 출력 (1, H) → 4-class 확률 벡터 (CPU)"""
...
def run_interactive(model, tokenizer, temp_warmup: int = 8, temp_max: float = 4.0):
    ...
    LSTM hidden state를 캐싱하여 이전 윈도우를 재계산하지 않음.
    ...
    hidden_state                   = None   # LSTM (h_n, c_n) 캐시
    processed_seg_count: int       = 0      # 이미 처리한 세그먼트 수
...
def main():
    parser = argparse.ArgumentParser(description="순차 윈도우 스트리밍 추론 (LSTM)")
