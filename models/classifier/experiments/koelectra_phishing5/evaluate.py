...
def resolve_checkpoint_path(run_id: str | None = None) -> tuple[Path, str]:
    if run_id is not None:
        return CHECKPOINT_DIR / f"koelectra_phishing5_{run_id}_best.pt", run_id
    latest_meta = CHECKPOINT_DIR / "koelectra_phishing5_latest.json"
...
def print_results(results: dict, split: str, run_id: str) -> None:
    print("\n" + "=" * 60)
    print(f"  koelectra_phishing5  split={split}  run={run_id}  ({results['num_samples']} samples)")
    print("=" * 60)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate koelectra_phishing5 on test/val split")
    ...
    save_path = LOG_DIR / f"koelectra_phishing5_{run_id}_{args.split}_{eval_run_id}_eval.json"
