...
[핵심 제약]
- 대화 초반부만 생성한다. 사기 시도의 도입부~압박 직전까지만 포함하라.
- 400~900자를 초과하면 안 된다. 간결하고 압축된 표현으로 작성하라.
...
def build_user_prompt(category: str, few_shots: list, n_generate: int) -> str:
    ...
    return (
        f"{cat_prompt}\n\n"
        f"[실제 STT 예시]\n{examples}\n\n"
        f"위 예시의 구어체 말투(망설임·반복·말 끊김 등 음성 전사 특성)를 따라 "
        f"동일 카테고리 보이스피싱 STT 전사 텍스트를 {n_generate}개 생성하라.\n"
        f"각 생성물은 발화 교환 5~8회 이내, 400~900자 이내로 간결하게 작성하라.\n"
