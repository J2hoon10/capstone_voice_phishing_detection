...
SNS 주제(subject) 분류 시나리오에서 Mamba hidden state가 어떻게 누적되는지:

```
t=1: 청크 c_1 = "이번 여름휴가 어디로 갈지 고민 중이야"
     BERT → H^(1): "여름휴가", "어디", "고민" 등 토큰 의미 인코딩
     Mamba: h_0(=0) + H^(1) → h_1  (여행 맥락 약하게 기록)
     p("여행") = 0.36  H = 1.04  → 불확실, 계속 청취

t=2: 청크 c_2 = "제주도 항공권 특가가 떴다던데?"
     BERT → H^(2): "제주도", "항공권", "특가" 등 토큰 인코딩
     Mamba: h_1 + H^(2) → h_2  (t=1 기억 + t=2 여행 단서 누적)
     p("여행") = 0.71  H = 0.63  → 확신 상승, 계속 청취

t=3: 청크 c_3 = "숙소 예약까지 끝내면 바로 떠날 수 있겠네"
     BERT → H^(3): "숙소", "예약", "떠나다" 등 핵심 토큰 인코딩
     Mamba: h_2 + H^(3) → h_3  (3개 청크 누적 → 강한 확신)
     p("여행") = 0.93  H = 0.22 < τ → 조기 종료
     ★ 최종 판정: 여행
```

**Mamba hidden state의 역할:**
- `h_1`: "여행 계획" 단서를 희미하게 기록
- `h_2`: "항공권 특가" 맥락 추가 누적 → 이전 기억(h_1)과 결합
- `h_3`: "숙소 예약" 단서가 h_2의 누적 기억과 결합 → 여행 주제 확신

> Mamba의 $\Delta$ (시간 스텝)가 각 청크에서 **얼마나 많은 정보를 흡수할지** 자동 결정.
> 주제 결정 단서가 강할수록 $\Delta$가 커져 상태 업데이트가 강해지는 효과.
...
```
예시 스트림: "올여름 휴가 계획 세워야지 | 제주도 항공권 알아봤어 | 숙소도 예약해두자"
정답 레이블: [여행]

t=1: p("여행")=0.34, p("주거와 생활")=0.24, p("식음료")=0.18   H=1.41 (불확실)
t=2: p("여행")=0.62, p("주거와 생활")=0.16, p("식음료")=0.10   H=1.05 (감소)
t=3: p("여행")=0.89, p("주거와 생활")=0.05, p("식음료")=0.03   H=0.43 (확신↑) → 판정
```
